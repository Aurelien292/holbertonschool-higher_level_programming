...
# Fonction pour récupérer les publications et les sauvegarder dans un fichier CSV
def fetch_and_save_posts():
    # Effectuer une requête GET pour récupérer les posts
    response = requests.get("https://jsonplaceholder.typicode.com/posts")
    
    # Si la requête a réussi (code 200), structurer les données dans une liste de dictionnaires
    if response.status_code == 200:
        # Convertir la réponse en objet JSON
        posts = response.json()
